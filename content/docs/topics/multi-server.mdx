...
description: This document explains how to setup a multi-server Tunnels environment
---
...
The control server manages all users, devices, and groups, while VPN servers handle the actual tunnel traffic. VPN servers validate user connections by verifying signatures from the control server.

## Server Installation

Follow [this guide](/docs/server/installation) to install the server on each Linux system (control server and VPN servers).<br/>
...
Edit the generated `config.json` to configure the control server:
