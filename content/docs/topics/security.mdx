...
**Distributed (Separate AUTH and VPN Servers)**
```
AUTH Server: CertPem/KeyPem
└─ PrivKey: Signs connection requests
...
The AUTH server handles authentication and signs connection approvals. VPN servers receive these signed approvals and verify them using the AUTH server's public key (loaded from `SignPem`).
...
- **Generation**: Self-signed via `certs.MakeCertV2()` in `certs/certs.go:163-284`

The private key (`PrivKey`) from KeyPem is used by the AUTH server to cryptographically sign connection approvals.

#### SignPem - AUTH Server Verification
- **Purpose**: VPN server verifies AUTH server signatures
- **Content**: AUTH server's public key/certificate
- **Used When**: AUTH and VPN are separate servers
- **Loading**: VPN server loads this at startup via `crypt.LoadPublicKey(SignPem)` in `server/main.go:311-319`

In single-server mode, `SignKey = PubKey` (verifies own signatures). In distributed mode, `SignKey` is loaded from SignPem (verifies AUTH server's signatures).

#### Certificate Distribution

Server certificates are distributed through the AUTH database:
...
**Distribution Flow:**
1. VPN server stores its CertPem in AUTH database's `Server.PubKey` field
2. Client queries AUTH for available VPN servers
3. Client receives server certificate and pins it
4. Client uses pinned certificate for all TLS connections

This eliminates the need for a certificate authority. Each VPN server's certificate is directly pinned by clients through the trusted AUTH server.
...
Signature algorithms in `crypt/keys.go:85-125` support both schemes.
...
User registration (`server/handlers.go:128-197`):
...
User login (`server/handlers.go:256-297`):
...
After authentication, the AUTH server signs connection requests (`server/handlers.go:1131-1217`):
...
The AUTH server signs with `PrivKey` (from its KeyPem), creating a cryptographic proof that this connection was authorized.

#### VPN Server Verification

The VPN server verifies the signature (`server/handlers.go:25-126`):
...
func API_AcceptUserConnections(w http.ResponseWriter, r *http.Request) {
    ...
    // 2. Verify AUTH server's signature
...
`SignKey` contains the AUTH server's public key:
- Single server: `SignKey = PubKey` (own public key)
- Distributed: `SignKey = LoadPublicKey(SignPem)` (AUTH's public key)

Request freshness validation prevents replay attacks by rejecting requests older than 4 minutes.

### Server Identity Verification

Clients verify server identity using certificate pinning and signature verification (`client/session.go:204-250`):
...
- Out-of-band certificate distribution through AUTH database
...
**Key Security Steps:**
- Step 2: AUTH signs with its `PrivKey`
...
The client generates ephemeral keys (`crypt/main.go:260-272`):
...
The server generates its own keys and creates shared secrets (`crypt/main.go:300-336`):
...
The client completes key agreement (`crypt/main.go:274-298`):
...
After key derivation, sensitive material is zeroed (`crypt/main.go:62-74`):
...
This cleanup is called on both client (`client/session.go:263`) and server (`server/handlers.go:117`) after the handshake completes.
...
Packets are encrypted with monotonically increasing nonces (`crypt/main.go:177-191`):
...
Decryption verifies authenticity (`crypt/main.go:193-210`):
