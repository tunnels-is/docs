...
Tunnels.is supports two deployment architectures:

1. **Single Server (AUTH + VPN combined)**
   - One server with CertPem/KeyPem
   - PrivKey signs connection requests
   - SignKey = PubKey (verifies own signatures)

2. **Distributed (AUTH server + VPN server)**
   - AUTH server: Has CertPem/KeyPem, signs with PrivKey
   - VPN server: Has its own CertPem/KeyPem for TLS, plus SignPem (AUTH's public key) to verify signatures

### Certificate Types

Tunnels.is uses two certificate/key configurations, stored as PEM-encoded files:

#### 1. **CertPem & KeyPem** - Server TLS Certificate and Signing Key
- **Location**: Configured in `config.json` or environment variables
- **Purpose**:
  - **CertPem (public certificate)**: TLS/HTTPS for API endpoints
  - **KeyPem (private key)**: TLS/HTTPS and signing connection requests
...
- **PrivKey** (from KeyPem) used to sign connection approvals from AUTH server

**Server Configuration:**
```go
// server/main.go:300-334
// Load private key and public certificate
priv, privB := crypt.LoadPrivateKey(KeyPem)
pub, pubB := crypt.LoadPublicKey(CertPem)
PrivKey = priv  // Used for signing connection requests
PubKey = pub    // Public key from certificate

// Create TLS certificate for HTTPS API
tlscert := tls.X509KeyPair(pubB, privB)
...
#### 2. **SignPem** - Controller Public Key for Verification
- **Purpose**: VPN server uses this to **verify** signatures from AUTH server
- **Used When**: AUTH and VPN features are on **separate** servers
- **Algorithm**: RSA or ECDSA public key (matches AUTH server's CertPem)
- **Format**: Public key certificate (PEM encoded)

**Key Concept:**
When the Controller (AUTH server) and VPN Server are separate entities:
- **AUTH server** signs connection approvals with its **PrivKey** (from its KeyPem)
- **VPN server** verifies signatures with **SignKey** (loaded from SignPem)
- SignPem contains the AUTH server's **public key/certificate**

**Key Loading Logic:**
```go
// server/main.go:311-319
if AUTHEnabled && VPNEnabled {
    // Same server handles both - use own public key for verification
    SignKey = PubKey
} else {
    // Separate servers - load AUTH server's public key
...
*Single Server (AUTH + VPN):*
```
Server has CertPem/KeyPem
├─ PrivKey: Signs connection requests
└─ SignKey = PubKey: Verifies own signatures
```

*Distributed (AUTH server + VPN server):*
```
AUTH Server has CertPem/KeyPem
└─ PrivKey: Signs connection requests

VPN Server has:
├─ Its own CertPem/KeyPem (for TLS)
└─ SignPem: AUTH server's public key for verification
```
...
Server certificates are distributed to clients through the AUTH server's database:

```go
// server/main.go:598-607
...
**Distribution Flow:**
1. VPN server generates CertPem/KeyPem during initialization
2. CertPem is stored in AUTH database's `Server.PubKey` field
3. Client queries AUTH server for available VPN servers
4. Client receives `Server.PubKey` containing the VPN server's certificate
5. Client pins this certificate for all TLS connections to that VPN server

**Security Model:**
- **Trust Anchor**: AUTH server is the trust anchor (must be accessed over HTTPS)
- **No CA Required**: Each VPN server's certificate is directly pinned by clients
- **Distributed Architecture**: Different VPN servers can have different certificates
- **Certificate Updates**: Updating server certificate requires database update
...
### Connection Request Signing (AUTH Server)

After authentication, the AUTH server signs connection requests:

```go
// server/handlers.go:1131-1217
func API_SessionCreate(w http.ResponseWriter, r *http.Request)
```

**AUTH Server Flow:**
1. **Receive Connection Request** (line 1134-1139)
   ```go
   CR := new(types.ControllerConnectRequest)
   // Contains: UserID, ServerID, DeviceToken, EncType, RequestingPorts
   ```

2. **Authenticate User** (line 1180-1184)
   ```go
   user, err := authenticateUserFromEmailOrIDAndToken("", CR.UserID, CR.DeviceToken)
   ```

3. **Check Authorization** (line 1185-1196)
   - Verify user/device is in allowed groups
   - Check server access permissions

4. **Sign Connection Request** (line 1199-1210)
   ```go
   SCR := new(types.SignedConnectRequest)
   CR.Created = time.Now()
   SCR.Payload = json.Marshal(CR)
   SCR.Signature = crypt.SignData(SCR.Payload, PrivKey)  // AUTH server signs with its private key
   ```

   **Signature Algorithm** (`crypt/keys.go:85-105`):
   - **RSA**: PSS padding with SHA-256, auto salt length
   - **ECDSA**: ASN.1 encoded with SHA-256

5. **Return Signed Request** (line 1212)
   Client receives signed payload to send to VPN server

### Connection Request Verification (VPN Server)

The VPN server verifies the AUTH server's signature:
...
1. **Receive Signed Request** (line 26-31)
   ...
   type SignedConnectRequest struct {
       Signature       []byte  // AUTH server's signature
       Payload         []byte  // Connection request
       X25519PeerPub   []byte  // Client's public key (added by client)
       Mlkem1024Encap  []byte  // Post-quantum key (added by client)
   }
   ```

2. **Verify AUTH Server Signature** (line 33-36)
   ```go
   err = crypt.VerifySignature(SCR.Payload, SCR.Signature, SignKey)
   ```

   **SignKey** is AUTH server's public key:
   - Same server: `SignKey = PubKey`
   - Distributed: `SignKey = LoadPublicKey(SignPem)`

   **Signature Algorithm** (`crypt/keys.go:107-125`):
   - **RSA**: PSS padding verification with SHA-256
   - **ECDSA**: ASN.1 verification with SHA-256
...
6. **Sign Server Response** (line 94-99)
   ...
   VPN server signs its public key with its own private key, allowing client to verify server identity.
...
- **Certificate Pinning**: Client receives and pins server's CertPem (stored in database `Server.PubKey` field)
- **Direct Trust Model**: No certificate authorities or PKI infrastructure required
- **Signature Verification**: Cryptographic proof of server identity
- **Out-of-Band Distribution**: Server certificates distributed through AUTH server database
- **Quantum-Resistant**: Post-quantum keys are signed with pinned certificates
...
[Client]                [AUTH Server]            [VPN Server]
   ...
   |    Mlkem1024Encap)                                 |
   |                          |                         |
   |                          |        [Verify Signature with SignKey]
   |                          |        [Validate Freshness]
   |                          |        [Initialize Encryption]
   |                          |        [Sign Response with PrivKey]
   |                          |                         |
   |<--4. Server Response------------------------<------|
   |   (X25519Pub, MLKEMCipher,                         |
   |    ServerHandshakeSignature, Config)               |
   |                          |                         |
...
**Key Security Steps:**
- **Step 2**: AUTH server signs with its **PrivKey** (from KeyPem)
- **Step 3**: VPN server verifies with **SignKey** (from SignPem or PubKey)
- **Step 4**: VPN server signs with its own **PrivKey** (from its KeyPem)
- **Step 5**: Client verifies VPN signature with **ServerPubKey** (pinned certificate)
...
1. **Certificate Management**
   ...
   - **Distributed Architecture**: VPN server's SignPem must contain AUTH server's public certificate
   - **Single Server**: SignPem not needed (SignKey uses own PubKey)
   - Monitor certificate expiration dates
   - Verify SignPem matches AUTH server's CertPem when setting up distributed deployments
