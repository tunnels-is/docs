...
DNS leaks occur when DNS queries bypass your VPN tunnel and go directly to your ISP's DNS servers, exposing your browsing activity.

### Required Configuration

To prevent DNS leaks when connected to Tunnels VPN:
...
1. Your application makes a DNS query → `127.0.0.1:53`
...
During VPN reconnections, Tunnels temporarily blocks all DNS queries to prevent leaks during the transition period.
...
The client automatically falls back to the backup DNS if the primary fails.

### Server-Side DNS Servers

The Tunnels VPN server can provide DNS servers to clients. The priority order is:

1. **Tunnel-specific DNS** (if configured on a specific tunnel)
2. **Server-provided DNS** (from the VPN server)
3. **Client default DNS** (your configured fallback)

This allows VPN servers to provide optimized or region-specific DNS servers while giving clients the flexibility to override them.
...
**URL Format:** Plain text file with one domain per line
**Local Lists:** If no URL is provided, place your list in the `blocklists/` directory with the tag name as the filename

### DNS Whitelists

Whitelists allow specific domains to bypass all blocklists. This is critical for preventing legitimate services from being blocked.
...
3. Test DNS leak at [dnsleaktest.com](https://dnsleaktest.com)
